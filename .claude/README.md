...
│   │
│   ├── dashboard/
│   │   └── dashboard.html           ← 태스크 대시보드 (브라우저에서 열기)
│   │
│   ├── settings.json                ← 공유 설정 (permissions, hooks)
│   └── settings.local.json          ← 로컬 설정 (SLACK_WEBHOOK_URL 등, git 제외)
│
└── output/
    ├── history.json                 ← 전체 태스크 이력
    └── tasks/
        ├── 001-대시보드-개선/
        │   ├── design.html            브라우저에서 확인
        │   ├── brief.md               dev 입력물
        │   └── complete.md            구현 결과
        ├── 002-가이드팝업-개선/
        │   └── ...
...
`.claude/dashboard/dashboard.html`을 브라우저에서 열면 태스크 현황을 확인할 수 있습니다.
